...
void PCD_WriteRegister(	byte reg,		///< The register to write to. One of the PCD_Register enums.
	...
	ENABLE_CHIP();		// Select slave
	spi_transmit(reg & 0x7E);				// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	spi_transmit(value);
	DISABLE_CHIP();		// Release slave again
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_WriteRegisterBuffer(	byte reg,		///< The register to write to. One of the PCD_Register enums.
...
byte PCD_ReadRegister(	byte reg	///< The register to read from. One of the PCD_Register enums.
	...
	byte value;
	ENABLE_CHIP();			// Select slave
	spi_transmit(0x80 | (reg & 0x7E));			// MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_ReadRegisterBuffer(	byte reg,		///< The register to read from. One of the PCD_Register enums.
	...
	spi_transmit(address);					// Tell MFRC522 which address we want to read
	while (index < count) {
		if (index == 0 && rxAlign) {		// Only update bit positions rxAlign..7 in values[0]
			...
			byte value = spi_transmit(address);
	...
	values[index] = spi_transmit(0);			// Read the final byte. Send 0 to stop reading.
	DISABLE_CHIP();			// Release slave again
} // End PCD_ReadRegister()
...
byte PCD_CalculateCRC(	byte *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
	...
	PCD_WriteRegisterBuffer(FIFODataReg, length, data);	// Write data to the FIFO
...
void PCD_Init() {
	...
	spi_init();
	if (IS_RESET()) {	//The MFRC522 chip is in power down mode.
		DISABLE_RESET(); // Exit power down mode. This triggers a hard reset.
